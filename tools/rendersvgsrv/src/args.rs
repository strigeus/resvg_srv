...
fn collect_args(args: Vec<OsString>) -> Result<CliArgs, pico_args::Error> {
    ...
    Ok(CliArgs {
        ...
        jpeg:               input.value_from_fn("--jpeg", parse_jpeg_level)?,
        free:               input.free()?,
    })
}

fn parse_jpeg_level(s: &str) -> Result<u32, String> {
    let n: u32 = s.parse().map_err(|_| "invalid number")?;

    if n <= 100 {
        Ok(n)
    } else {
        Err("JPEG level out of bounds".to_string())
    }
}
...
pub fn parse(args: Vec<OsString>) -> Result<(Args, resvg::Options), String> {
    let args = collect_args(args).map_err(|e| e.to_string())?;

    if args.help {
        return Err(print_help());
    }
